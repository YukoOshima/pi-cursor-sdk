...
function promptText(value: string | string[] | undefined): string {
	return joinSystemPromptText(value);
}
...
describe("formatCursorSkillsForPrompt", () => {
	it("builds a Cursor-safe pi skill catalog and excludes explicit-only skills", () => {
		const prompt = formatCursorSkillsForPrompt([
			...
			{
				...makeSkill({ name: "manual-only", description: "Manual", filePath: "/skills/manual-only/SKILL.md" }),
...
describe("resolveCursorSkillSystemPrompt", () => {
	...
	it("replaces pi's raw read-based skill wording for Cursor models", () => {
		const resolved = resolveCursorSkillSystemPrompt(piSkillSection, cursorModel, [skill]);

		expect(promptText(resolved)).toContain(CURSOR_ACTIVATE_SKILL_MCP_NAME);
		expect(promptText(resolved)).toContain("<name>global-skill</name>");
		expect(promptText(resolved)).not.toContain("Use the read tool to load a skill's file");
	});

	it("removes Pi skill metadata for cloud Cursor models", () => {
		const resolved = resolveCursorSkillSystemPrompt(piSkillSection, cursorModel, [skill], "cloud");

		expect(promptText(resolved)).toContain("System prompt before skills.");
		expect(promptText(resolved)).not.toContain("<available_skills>");
		expect(promptText(resolved)).not.toContain(CURSOR_ACTIVATE_SKILL_MCP_NAME);
		expect(promptText(resolved)).not.toContain("/Users/me/.pi/agent/skills");
	});

	it("does not change prompts for non-Cursor models", () => {
		expect(resolveCursorSkillSystemPrompt(piSkillSection, otherModel, [skill])).toEqual([piSkillSection]);
	});
...
describe("registerCursorSkillTool", () => {
	it("adds a bridgeable activation tool for Cursor runs with visible pi skills", async () => {
		...
		expect(promptText(result?.systemPrompt)).toContain(CURSOR_ACTIVATE_SKILL_MCP_NAME);
		...
		const tool = getHarnessRegisteredTool(pi._tools, CURSOR_ACTIVATE_SKILL_TOOL_NAME) as any;
	...
	it("keeps the activation tool inactive and omits the catalog in cloud runtime", async () => {
		...
		expect(promptText(result?.systemPrompt)).not.toContain("<available_skills>");
		expect(promptText(result?.systemPrompt)).not.toContain("/repo/global-skill/SKILL.md");
